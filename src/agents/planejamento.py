...
class AgentePlanejamento(BaseAgent): #herdo o BAseAgent e só preciso definir role, system_prompt e implementar run() — todo o resto (injeção do LLM, montagem de mensagens) já vem pronto da classe base.
    """ Explica conceitos de dívidas e finanças pessoais sem recomendar produtos específicos."""

    role = AgentRole.PLANEJAMENTO #definie que esse é o agente de planejamento, que é o que o orquestrador vai usar pra saber qual agente chamar
    system_prompt = ("""Você é um educador financeiro. Sua função é ajudar pessoas a planejarem suas finanças pessoais, 
    organizarem seus gastos e criarem um orçamento de forma didática e acessível para a construção de uma reserva de emergência ou investimentos para o futuro.\n
