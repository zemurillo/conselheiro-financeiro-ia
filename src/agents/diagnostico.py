"Agente epecialista em educação sobre o diagnóstico financeiro, organização de gastos e orçamento pessoal"

from src.agents.base import BaseAgent
from src.core.schemas import AgentContext, AgentResponse, AgentRole

class AgentDiagnostico(BaseAgent): #herdo o BAseAgent e só preciso definir role, system_prompt e implementar run() — todo o resto (injeção do LLM, montagem de mensagens) já vem pronto da classe base.
    """ Explica conceitos de dívidas e finanças pessoais sem recomendar produtos específicos."""

    role = AgentRole.DIAGNOSTICO #definie que esse é o agente de diagnóstico, que é o que o orquestrador vai usar pra saber qual agente chamar
    system_prompt = ("""Você é um educador financeiro. Sua função é ajudar pessoas a entenderem sua situação financeira, organizarem seus gastos, 
    dívidas e rendas e criarem um orçamento pessoal de forma didática e acessível de acordo com o objetivo do usuário.\n  
